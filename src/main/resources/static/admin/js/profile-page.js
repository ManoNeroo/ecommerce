(function () {
    const CHANGE_AVATAR_BTN = document.querySelector(".user-avatar .change-avatar-btn");
    const USER_FORM = document.getElementById("user-basic-info-form");
    const USER_PASSWORD_FORM = document.getElementById("user-password-form");
    const PHONE_INPUT = USER_FORM && USER_FORM.querySelector("input[name='phoneNumber']");
    ...
    USER_FORM && USER_FORM.querySelectorAll("input[type='text']").forEach(input => {
    ...
    USER_FORM && USER_FORM.addEventListener("submit", handleUpdateBasicInfo);
    ...
    async function handleUpdateBasicInfo(evt) {
        evt.preventDefault();
        if (validateForm(USER_FORM)) {
            const formData = getFormData(USER_FORM);
            const resp = await httpClient("/api/account/basicinfo/" + formData.userId, "PUT", formData);
            if (resp.isSuccess) {
                window.location.href = window.location.origin + "/admin/resetusersession?returnUrl=/admin/profile";
            } else {
